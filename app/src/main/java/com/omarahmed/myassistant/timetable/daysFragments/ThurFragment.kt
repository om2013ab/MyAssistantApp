...
import com.omarahmed.myassistant.databinding.FragmentThurBinding
...
class ThurFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        binding = FragmentThurBinding.inflate(inflater)
        binding.fabThursday.setOnClickListener {
        ...
        timetableViewModel.getSchedule("Thursday").observe(viewLifecycleOwner, Observer {
            timetableAdapter.scheduleList.submitList(it)
        })
        binding.rvThursday.adapter = timetableAdapter
        return binding.root
    }
    ...
    private fun insertSchedule(view: DialogAddSchedualBinding, dialog: AlertDialog) {
        ...
        val newSchedule = TimetableInfo(0, code, from, to, venue, "Thursday")
